 ...
 * Returns an invalid Date object if `iso_string` is null or undefined.
 */
function parseISOString(iso_string) {
    if (iso_string == null || iso_string == undefined) {
    ...
    user_scores.sort( function(a, b) {
        ...
        // If the scores are equal, then rank the player which has found a Korok the earliest higher
            ...
            try {
                ...
                const a_millis = a_time_of_last_scan.getTime();
                const b_millis = b_time_of_last_scan.getTime();
                // Sort player later if their time of last scan is undefined
                if (isNaN(a_millis) && isNaN(b_millis)) {
                    return 0;
                } else if (isNaN(a_millis)) {
                    return 1;
                } else if (isNaN(b_millis)) {
                    return -1;
                }

                return a_millis - b_millis;
            } catch (e) {
                // Just sort randomly if there is an error
                return 0;
            }
        }
    });
